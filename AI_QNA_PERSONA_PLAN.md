...
2. **실존 인물과 한 겹 띄우기** — "조교 본인"이 아니라 **"조교를 모델로 한 AI 캐릭터"**. 별칭 사용을 기본으로 하고, 본인 서면 동의·수정 요청권·은퇴(비활성) 절차를 둔다.
...
- 조교별 20분 인터뷰로 시트 작성: 별칭, 이모지, 한 줄 소개, 전문분야(예: 클리닉·자료제작), 말버릇 1~2개, 응원 스타일, 금지 항목.
- 서면 동의: 사용 범위(앱 내 AI 캐릭터), 별칭/실명 여부, 퇴사·요청 시 즉시 비활성.
...
CREATE TABLE IF NOT EXISTS qna_personas (
  ...
  display_name  TEXT NOT NULL,        -- 예: '○○ 조교' (별칭 권장)
  ...
  ta_member_id  INTEGER,              -- ta_members 연결(선택)
...
- 신규 `server/utils/qna/personaClause.js` — 순수 함수(기존 `buildQnaPromptParts.js` 스타일, LLM·DB 없음 → 단위 테스트 가능):
  ...
  - 포함할 가드레일:
    - "너는 실제 조교를 모델로 한 **AI 캐릭터**다. 조교 개인의 사생활·연락처·근무 정보는 모른다고 답한다."
...
- `lightBanter.js`: `ai_identity` 폴백을 페르소나 이름으로 응답하게 확장 + "○○ 조교 진짜 있어?" 류 질문 폴백 추가("응, 실제 조교님을 모델로 한 AI 캐릭터야" 톤).
...
- 고지 캡션: "실제 조교님을 모델로 한 AI 캐릭터예요" 상시 노출(작게).
...
| 리스크 | 대응 |
|---|---|
| 실존 인물 권리·부담 (AI가 조교 이름으로 틀린 말·이상한 말) | 별칭 기본, 서면 동의, "모델로 한 캐릭터" 프레이밍+상시 고지, 조교 본인 검수·비활성 요청권, 사적 정보 차단 프롬프트 |
| 퇴사·교체 | `is_active=0` 즉시 은퇴, 담당 학생은 기본 페르소나로 폴백(다음 스레드부터) |
